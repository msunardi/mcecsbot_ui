...
- (void)writeEnteredText:(NSString *)inputText {
    ...
    NSLog(@"text length: %d",[inputText length]);
    NSString *response = [NSString stringWithFormat:@"ipad:%@", inputText];
    NSData *data = [[NSData alloc] initWithData:[response dataUsingEncoding:NSASCIIStringEncoding]];
    [outputStream write:[data bytes] maxLength:[data length]];
    self.chatViewInputText.text = @""; // clears the input field
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (cell == nil) {
        ...
        cell.backgroundView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"chatbg.png"]];
...
- (void)initNetworkCommunication {
    ...
    CFStreamCreatePairWithSocketToHost(NULL, (CFStringRef)CFBridgingRetain(self.chatServer), 8008, &readStream, &writeStream);
